...
class CMRGranule:
    ...
    def __init__(self, query_result: dict, provider: str):
        ...
        self.mod_time = datetime.strptime(
            query_result.get("updated"), "%Y-%m-%dT%H:%M:%S.%fZ"
        )
    ...
    def extract_url(self, links: Iterable[dict], provider: str) -> str:
        for link in links:
            if (
                "rel" in link
                and link["rel"] == "http://esipfed.org/ns/fedsearch/1.1/data#"
            ):
...
class CMRQuery:
    ...
    def query(self) -> Iterable[CMRGranule]:
        ...
        logger.info(
            f"Found {len(cmr_granules)} granule(s) from CMR query. Checking for updates..."
        )
        return cmr_granules
