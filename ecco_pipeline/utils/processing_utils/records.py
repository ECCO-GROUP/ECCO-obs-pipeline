...
class TimeBound():
    ...
    def __init__(self, rec_avg_start: np.datetime64|None=None, rec_avg_end: np.datetime64|None=None, period: str='AVG_DAY'):
        
        if all([rec_avg_start, rec_avg_end]) or None not in [rec_avg_end, rec_avg_start]:
            raise ValueError(f'One of rec_avg_start or rec_avg_end must be provided, but not both.')
        
        if period not in ['AVG_MON', 'AVG_DAY', 'AVG_WEEK', 'AVG_YEAR']:
            raise ValueError(f'{period} is invalid output_freq_code. Must be one of AVG_MON, AVG_DAY, AVG_WEEK, OR AVG_YEAR')
