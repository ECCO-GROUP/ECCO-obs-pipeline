...
def get_dataset_list(config_dir):
...
def report_file_counts(root_path, config_path):
    """
    ...
    dataset_configs = get_dataset_list(config_path)

    # --- Step 1: Determine full set of grids across all datasets ---
    all_grids = set()
    for ds_name in dataset_configs:
        transformed_path = root_path / ds_name / "transformed_products"
    ...
    for ds_name in dataset_configs:
        ...
        for stage in ["Transform", "Aggregate"]:
            for grid in all_grids:
                ...
                if stage == "Transform":
                    count = sum(1 for f in grid_path.rglob("transformed/*/*.nc")) if grid_path and grid_path.exists() else 0
                else:
                    count = sum(1 for f in grid_path.rglob("aggregated/*/netCDF/*.nc")) if grid_path and grid_path.exists() else 0
                records.append({
                    "dataset": ds_name,
                    "stage": stage,
                    "grid": grid,
...
def pivot_file_counts(df):
    ...
    df_report = df_report.sort_values(["dataset", "stage_order"]).drop(columns=["stage_order", "stage_y"]).rename(columns={"stage_x": "stage"}).reset_index(drop=True)
...
config_dir = "ecco_pipeline/conf/ds_configs"
df_pipeline = report_file_counts(output_dir, config_dir)
