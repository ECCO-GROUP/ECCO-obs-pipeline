    ...
    for ds in datasets:
        ...
        for g in grids:
            ...
            for var in vars:
                ...
                fp = Path(
                    ds / "transformed_products" / g / "aggregated" / var / "netCDF"
                )
...
def export_ds_found_dict(
    ...
    if type(export_filename) is str and len(export_filename) > 0:
        try:
    ...
    print("===================================================")
    print("ECCO-pipeline Aggregated Observations Report")
    ...
    for ds in ds_found.keys():
        ...
        for g in ds_found[ds].keys():
            ...
            for v in ds_found[ds][g].keys():
                ...
                print(
                    f"     * {v.ljust(28)}   MON:{str(len(tmp['mon_files'])).ljust(2)} {mflys}  DAY:{str(len(tmp['day_files'])).ljust(2)} {dflys}"
                )
        print("\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-od", "--observations_dir", help="root 'observations' directory", required=True
    )
    parser.add_argument(
        "-dcd", "--ds_conf_dir", help="directory with ds_conf yamls", required=True
    )
    parser.add_argument(
        "-ef",
        "--export_to_file",
    ...
    parser.add_argument(
        "-rfn",
        "--export_filename",
        help="filename to export report to",
        required=False,
    ...
    parser.add_argument(
        "-ds",
        "--date_stamp",
        help="add date stamp to export_filename",
        required=False,
        type=bool,
        default=True,
    )
